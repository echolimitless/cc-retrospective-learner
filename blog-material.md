...
### 「MEMORY.md の肥大化対策」
auto memory の MEMORY.md は毎回システムプロンプトに読み込まれ、200行を超えると切り捨てられる。この仕組みでファイルが増え続けると、後ろのエントリが見えなくなるリスクがある。

対策として:
- short-term/ と long-term/ は MEMORY.md のインデックスに載せない。session-reviewer と weekly-promoter がディレクトリを直接 Glob で探索するので目次不要
- MEMORY.md に載せるのは feedback_*.md と user_*.md だけ。これらは本当に定着したパターンだけが作られるので急激には増えない（200個超えるには数ヶ月〜年単位）
- サブエージェントは MEMORY.md に依存せずディレクトリを直接探索する設計にしたため、仮に MEMORY.md から溢れても仕組み自体は動き続ける
- MEMORY.md が肥大化した場合は、Claude Code が auto memory の仕組みとして自分で整理する

この設計により「進化メモリの仕組み自体が MEMORY.md の制約に縛られない」という独立性を確保している。
